...
static __inline__ char __heap_is_aligned(uintptr_t value, uintptr_t alignment)
{
...
static __inline__ char __heap_prev_used(const heap_block *block)
{
	return (block->size&HEAP_BLOCK_PREV_USED);
}

static __inline__ char __heap_is_used(const heap_block *block)
{
	const heap_block *const next_block = __heap_block_at(block, __heap_block_size(block));
	return __heap_prev_used(next_block);
}

static __inline__ char __heap_is_free(const heap_block *block)
{
	return !__heap_is_used(block);
}

static __inline__ char __heap_block_in(const heap_cntrl *theheap, const heap_block *block)
{
